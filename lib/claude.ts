...
- Only call real, free, no-auth-required public APIs. Known working examples: wttr.in (weather), api.coinbase.com/v2/prices/BTC-USD/spot (crypto price — preferred, no rate limits), numbersapi.com/:number (trivia), api.adviceslip.com/advice (advice).
- For crypto prices always use api.coinbase.com/v2/prices/{PAIR}/spot (e.g. BTC-USD, ETH-USD). NEVER use coingecko — it rate-limits aggressively on shared IPs.
